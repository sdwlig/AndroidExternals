...
typedef long (*BIO_callback_fn_ex)(BIO* b, int oper, const char* argp, size_t len, int argi,
								   long argl, int ret, size_t* processed);
BIO_callback_fn_ex BIO_get_callback_ex(const BIO* b);
void               BIO_set_callback_ex(BIO* b, BIO_callback_fn_ex callback);
long BIO_debug_callback_ex(BIO* bio, int oper, const char* argp, size_t len, int argi, long argl,
...
#define sk_BIO_new_reserve(cmp, n) \
	((STACK_OF(BIO)*)OPENSSL_sk_new_reserve(ossl_check_BIO_compfunc_type(cmp), (n)))
...
#define sk_BIO_delete_ptr(sk, ptr) \
	((BIO*)OPENSSL_sk_delete_ptr(ossl_check_BIO_sk_type(sk), ossl_check_BIO_type(ptr)))
#define sk_BIO_push(sk, ptr) OPENSSL_sk_push(ossl_check_BIO_sk_type(sk), ossl_check_BIO_type(ptr))
#define sk_BIO_unshift(sk, ptr) \
...
#define sk_BIO_deep_copy(sk, copyfunc, freefunc)                                                                    \
	((STACK_OF(BIO)*)OPENSSL_sk_deep_copy(ossl_check_const_BIO_sk_type(sk), ossl_check_BIO_copyfunc_type(copyfunc), \
										  ossl_check_BIO_freefunc_type(freefunc)))
#define sk_BIO_set_cmp_func(sk, cmp) \
	((sk_BIO_compfunc)OPENSSL_sk_set_cmp_func(ossl_check_BIO_sk_type(sk), ossl_check_BIO_compfunc_type(cmp)))
...
#define BIO_write_filename(b, name) \
	(int)BIO_ctrl(b, BIO_C_SET_FILENAME, BIO_CLOSE | BIO_FP_WRITE, name)
#define BIO_append_filename(b, name) \
	(int)BIO_ctrl(b, BIO_C_SET_FILENAME, BIO_CLOSE | BIO_FP_APPEND, name)
#define BIO_rw_filename(b, name) \
	(int)BIO_ctrl(b, BIO_C_SET_FILENAME, BIO_CLOSE | BIO_FP_READ | BIO_FP_WRITE, name)
...
int BIO_dgram_sctp_notification_cb(BIO* b, BIO_dgram_sctp_notification_handler_fn handle_notifications,
								   void* context);
...
int BIO_dump_indent_cb(int (*cb)(const void* data, size_t len, void* u), void* u, const void* s,
					   int len, int indent);
...
int BIO_lookup(const char* host, const char* service, enum BIO_lookup_type lookup_type, int family,
			   int socktype, BIO_ADDRINFO** res);
int BIO_lookup_ex(const char* host, const char* service, int lookup_type, int family, int socktype,
				  int protocol, BIO_ADDRINFO** res);
...
#if defined(__GNUC__) && defined(__STDC_VERSION__) && !defined(__MINGW32__) \
