...
int    OPENSSL_buf2hexstr_ex(char* str, size_t str_n, size_t* strlength, const unsigned char* buf,
							 size_t buflen, const char sep);
...
#define sk_void_new_reserve(cmp, n) \
	((STACK_OF(void)*)OPENSSL_sk_new_reserve(ossl_check_void_compfunc_type(cmp), (n)))
...
#define sk_void_delete_ptr(sk, ptr) \
	((void*)OPENSSL_sk_delete_ptr(ossl_check_void_sk_type(sk), ossl_check_void_type(ptr)))
#define sk_void_push(sk, ptr) \
	OPENSSL_sk_push(ossl_check_void_sk_type(sk), ossl_check_void_type(ptr))
#define sk_void_unshift(sk, ptr) \
...
#define sk_void_deep_copy(sk, copyfunc, freefunc)                                   \
	((STACK_OF(void)*)OPENSSL_sk_deep_copy(ossl_check_const_void_sk_type(sk),       \
										   ossl_check_void_copyfunc_type(copyfunc), \
										   ossl_check_void_freefunc_type(freefunc)))
#define sk_void_set_cmp_func(sk, cmp) \
	((sk_void_compfunc)OPENSSL_sk_set_cmp_func(ossl_check_void_sk_type(sk), ossl_check_void_compfunc_type(cmp)))
...
typedef int CRYPTO_EX_dup(CRYPTO_EX_DATA* to, const CRYPTO_EX_DATA* from, void** from_d, int idx,
						  long argl, void* argp);
__owur int  CRYPTO_get_ex_new_index(int class_index, long argl, void* argp, CRYPTO_EX_new* new_func,
...
OSSL_DEPRECATEDIN_3_0 void CRYPTO_mem_debug_realloc(void* addr1, void* addr2, size_t num, int flag,
